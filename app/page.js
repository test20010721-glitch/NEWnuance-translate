...
const s = {
  ...
  listeningBanner: { background: '#e8f0fe', margin: '0 14px 10px', borderRadius: 8, padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  listeningText: { fontSize: 13, color: '#1a56db', fontWeight: 500 },
  stopBtn: { fontSize: 11, color: '#1a56db', background: 'none', border: '1px solid #c5d5f8', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', fontFamily: 'inherit' },
...
// スマホの大きな画像をリサイズしてJPEG base64に変換
...
export default function Home() {
  ...
  const isSameMode = srcLang === tgtLang  // スタイル変換モード
  ...
  const translate = async (text = inputText) => {
    ...
    try {
      ...
      saveHistory([...history, {
        ...
        srcLang: LANGS[srcLang]?.src || srcLang, tgtLang: LANGS[tgtLang]?.src || tgtLang,
  ...
  const handleSrcLang = (v) => { setSrcLang(v); if (v === tgtLang) setTgtLang(v === 'ja' ? 'en' : 'ja') }
  const handleTgtLang = (v) => { setTgtLang(v); if (v === srcLang) setSrcLang(v === 'ja' ? 'en' : 'ja') }
  ...
  const handleMic = () => {
    ...
    if (!SR) {
      alert('音声入力に対応していません。\nChromeまたはSafari（iOS 14.5以上）をお使いください。')
      return
    }
    ...
    rec.continuous = false       // iOSで安定動作するようfalseに設定
    ...
    rec.onerror = (e) => {
      ...
      else if (e.error === 'network') alert('音声認識にはインターネット接続が必要です。')
      else if (e.error === 'no-speech') setInputText('')  // 無音の場合は静かに処理
      setIsListening(false)
    }

    rec.onend = () => setIsListening(false)

    try { rec.start() } catch(e) { alert('音声入力を開始できませんでした。'); setIsListening(false) }
  }
  ...
  const handleCam = () => {
    ...
    input.type = 'file'
    input.accept = 'image/*'
    input.capture = 'environment'  // スマホのリアカメラを優先
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (data.error) throw new Error(data.error)
        if (!data.result?.trim()) {
          setResult('文字が検出できませんでした。文字がはっきり写った画像をお試しください。')
          return
        }
        setResult(data.result)
  ...
  return (
    <div style={s.app}>
      ...
      <div style={s.langBar}>
        <select style={s.select} value={srcLang} onChange={e => handleSrcLang(e.target.value)}>
        ...
        <select style={s.select} value={tgtLang} onChange={e => handleTgtLang(e.target.value)}>
          <option value="en">English</option>
          <option value="ja">日本語</option>
          <option value="es">Español</option>
          <option value="ca">Català</option>
      ...
      {isSameMode && (
        <div style={{ background: '#fef9ec', border: '1px solid #fde68a', borderRadius: 10, padding: '8px 14px', fontSize: 13, color: '#92400e' }}>
          💡 同じ言語が選ばれています。上のトーンを選ぶと<strong>スタイル変換</strong>として動作します（例：メール用→ビジネスメール作成、メニュー→料理説明）
        </div>
      )}
      ...
      <div style={s.card}>
        ...
        {isListening && (
          <div style={s.listeningBanner}>
            <span style={{ ...s.listeningText, animation: 'pulse 1.5s infinite' }}>{L.listening}</span>
            <button style={s.stopBtn} onClick={() => recognitionRef.current?.stop()}>{L.tapStop}</button>
          </div>
        )}
